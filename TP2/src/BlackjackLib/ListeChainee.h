...
public:
    ...
    void TrierAscendant();
    T Obtenir(int);
    void SauveGarder(const std::string& p_nomFichier)
    {
    ...
    template<typename T>
    friend std::istream& operator>>(std::istream& in, ListeChainee<T>& p_liste);
    //{
    //    T valeur;
    //    while (in >> valeur)
    //    {
    //        p_liste.AjouterFin(valeur);
    //    }
    //    return in;
    //}
...
template<typename T>
std::istream& operator>>(std::istream& in, ListeChainee<T>& p_liste)
{
    T valeur;
    while (in >> valeur)
    {
